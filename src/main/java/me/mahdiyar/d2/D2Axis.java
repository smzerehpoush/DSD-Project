...
public class D2Axis {
    ...
    private D2DataWrapper data;
    private boolean labelXUp;
    private boolean labelYLeft;
    private boolean arrowUp;
    private boolean arrowLeft;
    private D2IntRange xLogRange;
    private D2IntRange yLogRange;

    public D2Axis(D2PlotCanvas plotcanvas) {
        canvas = plotcanvas;
    ...
    public void draw(Graphics g) {
        ...
        D2Format format = data.getFormatX();
        D2Format format1 = data.getFormatY();
    ...
    private void drawX(Graphics g, D2Format format) {
        ...
        if (arrowLeft) {
            D2Plot.drawArrowLeft(0, origin.y, 4, g);
            g.drawString(s, 1, origin.y + j);
        } else {
            D2Plot.drawArrowRight(i, origin.y, 4, g);
    ...
    private void tickLinX(Graphics g, D2Format format, int i, int j, int k) {
        FontMetrics fontmetrics = g.getFontMetrics();
        D2FloatRange floatrange = data.getRangeX();
    ...
    private void drawY(Graphics g, D2Format format) {
        ...
        if (k == 0)
            k = 2;
        if (arrowUp) {
            D2Plot.drawArrowUp(origin.x, 0, 4, g);
            g.drawString(s, origin.x + k, j);
        } else {
            D2Plot.drawArrowDown(origin.x, i, 4, g);
    ...
    private void tickLinY(Graphics g, D2Format format, int i, int j, int k) {
        FontMetrics fontmetrics = g.getFontMetrics();
        D2FloatRange floatrange = data.getRangeY();
    ...
    private void initX(FontMetrics fontmetrics, D2Format format) {
        D2FloatRange floatrange = data.getRangeY();
        String s = data.getNameY();
    ...
    private void initLinX(int i) {
        ...
        D2FloatRange floatrange = data.getRangeX();
        D2FloatPoint floatpoint = data.getOrigin();
        int k;
        int l;
        if (floatrange.min >= 0.0D) {
            k = 0;
